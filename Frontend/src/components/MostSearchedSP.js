...
function MostSearchedSP() {
  ...
  const { data: products } = useSelector((state) => state.slideproducts);
  ...
  const mostSearchedPhones = useMemo(() => {
    const trackedProducts = Array.isArray(data) ? data : [];
    const catalogProducts = Array.isArray(products) ? products : [];
    const usedIds = new Set(
      trackedProducts
        .map((item) => String(item?.productId || item?._id || ""))
        .filter(Boolean)
    );
    ...
    const curatedFallback = catalogProducts
      .filter((item) => {
        const category = String(item?.category || "");
        const productName = String(item?.product_name || "");
        return (
          !usedIds.has(String(item?._id || "")) &&
          (/smart/i.test(category) || smartphoneMatcher.test(productName))
        );
      })
      .sort((left, right) => {
        const leftName = String(left?.product_name || "");
        const rightName = String(right?.product_name || "");
        const flagshipBoost = /(s25|ultra|iphone 17|iphone 17 pro|pro max|pixel)/i;
        const leftScore = flagshipBoost.test(leftName) ? 1 : 0;
        const rightScore = flagshipBoost.test(rightName) ? 1 : 0;
    ...
    return [...trackedProducts, ...curatedFallback].slice(0, 6);
  }, [data, products]);
    ...
    <section className="relative overflow-hidden px-3 py-12 sm:px-6 sm:py-16 lg:px-10 lg:py-24">
      ...
      <div className="relative mx-auto max-w-7xl">
        ...
        <div className="mb-6 overflow-x-auto pb-3 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:mb-8 sm:overflow-visible sm:pb-0">
          <div className="flex w-max gap-4 px-1.5 sm:grid sm:w-auto sm:grid-cols-2 sm:gap-5 sm:px-0 xl:grid-cols-3">
            {mostSearchedPhones.map((item, index) => (
              <Link
                key={`${item.productId}-${index}`}
                to={`/product/${item.productId}`}
                onClick={() => handleProductOpen(item.productId)}
                className="group relative w-[82vw] min-w-[82vw] max-w-[286px] snap-start overflow-hidden rounded-[24px] border border-white/70 bg-white/90 shadow-[0_14px_40px_rgba(15,23,42,0.08)] backdrop-blur-sm transition duration-500 hover:-translate-y-2 hover:shadow-[0_26px_90px_rgba(15,23,42,0.14)] sm:w-auto sm:min-w-0 sm:max-w-none sm:rounded-[28px] sm:shadow-[0_18px_70px_rgba(15,23,42,0.08)]"
              >
                ...
                <div className="p-3.5 sm:p-4">
                  <div className="relative overflow-hidden rounded-[18px] bg-[linear-gradient(180deg,#fff7ed_0%,#ffffff_100%)] sm:rounded-[22px]">
                    <div
                      className="h-[220px] bg-contain bg-center bg-no-repeat transition duration-700 group-hover:scale-[1.04] sm:h-[240px]"
                  ...
                  <div className="mt-4 flex items-start justify-between gap-3 sm:mt-5">
                    <div>
                      <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-400 sm:text-xs sm:tracking-[0.24em]">
                        {item.source === "fallback" ? "Popular pick" : "Most searched"}
                      </p>
